...
void Rocket::Update(float dt)
{
    ...
    swaws::vec2 force = direction.Rotate(swaws::math::DegToRad(owner->transform.rotation)) * owner->speed;
    ...
    owner->transform.position.x = swaws::math::wrap((float)owner->transform.position.x, (float)0, (float)swaws::GetEngine().GetRenderer().GetWindowWidth());
    owner->transform.position.y = swaws::math::wrap((float)owner->transform.position.y, (float)0, (float)swaws::GetEngine().GetRenderer().GetWindowHeight());
